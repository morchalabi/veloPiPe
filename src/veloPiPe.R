...
for(btch_ in btchs_)
{
  ...
  bamfiles_ = bamfiles_[!grepl(x = bamfiles_, pattern = '*.bai\\b')]                                  # filtering our bai files
  ...
  rslt_btch = mclapply(X = bamfiles_, mc.cores = cpu_, genes_tbl, exons_tbl,  FUN =
                                                                              function(bamfile_, genes_tbl, exons_tbl)
                                                                              {
                                                                                ...
                                                                                spliced_vec = unspliced_vec = integer(length = nrow(genes_tbl))     # vector containing spliced counts
                                                                                names(spliced_vec) = names(unspliced_vec) = rownames(genes_tbl)     # vector containing unspliced counts
                                                                                ...
                                                                                for(seq_ in unique(aligns_$seq))                                    # for each sequence
                                                                                {
                                                                                  ...
                                                                                  for(rw_ in 1:nrow(aligns_sub))                          # for each alignment
                                                                                  {
                                                                                    ...
                                                                                    if(nrow(align_str_exons) == 0 & nrow(align_end_exons) == 0)
                                                                                    {
                                                                                      ...
                                                                                      if(length(gene_) == 1){ spliced_vec[gene_] = spliced_vec[gene_] + 1 }                                         # alignment is in intronic region of a gene
                                                                                      next()
                                                                                    }
                                                                                    ...
                                                                                    if(nrow(align_str_exons) != 0 & nrow(align_end_exons) != 0)
                                                                                    {
                                                                                      ...
                                                                                      if(length(cmn_exons) != 0)                                                    # there is at least one exon crossed by both ends of current alignment
                                                                                      {
                                                                                      ...
                                                                                      {
                                                                                        spliced_vec[genes_] = spliced_vec[genes_] + 1
                                                                                      }
                                                                                      next()
                                                                                    }
                                                                                ...
                                                                                return( cbind(unspliced = unspliced_vec, spliced = spliced_vec))
                                                                              })
  # STEP 4: Writing matrices to disk ####
  
  names(rslt_btch) = cells_
  spliced_mat = unspliced_mat = Matrix(data = 0, nrow = nrow(genes_tbl), ncol = length(cells_), sparse = T,
